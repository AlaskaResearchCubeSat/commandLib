function SPI_write(sobj,dest,sector,data)
...
%          bytes before sending so make sure that it is in the propper form
    ...
    %send command
    command(sobj,'SPIdread %s %lu %u',dest,sector,length(data)/512);
    ...
    umsg='unknown command ''SPI''';
    %check for unknown command
    if(strncmpi(umsg,line,length(umsg)))
        %wait for command to finish
        waitReady(sobj);
        error('SPI command not avaliable on microcontroller');
    end
